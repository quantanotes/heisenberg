...
HNSW loadHNSW(char *path, int dim, int space_type)
{
    hnswlib::HierarchicalNSW<float> *hnsw;
    hnswlib::SpaceInterface<float> *space = getSpace(space_type, dim);
    try
    {
        hnsw = new hnswlib::HierarchicalNSW<float>(space, std::string(path), false, 0, true);
    }
...
void addPoint(HNSW hnsw, float *vec, unsigned long int id, float dist)
{
...
int *search(HNSW hnsw, float *vec, int k)
{
